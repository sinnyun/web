...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (slideshowContainer) {
        ...
        if (typeof projects !== 'undefined') {
            // 指定要显示的项目ID顺序
            const projectIds = [1, 3, 2, 4, 6, 5];
            const slideshow = new Slideshow(slideshowContainer, {
                projects: projectIds
            });
            console.log('Slideshow initialized with projects:', projectIds);
        } else {
...
// 为所有可交互元素添加鼠标效果
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('a, button, .menu-button, .slide-arrow, .close-menu-button, .nav-link, .indicator, .thumbnails-container').forEach(element => {
        element.addEventListener('mouseenter', handleMouseEnter);
        element.addEventListener('mouseleave', handleMouseLeave);
    });
});
